...
async def bot_start(message: types.Message):
    await message.answer(f"Salom, {message.from_user.full_name}\n Kampyuterga tuwiw uchun ruxsat botiga hush kelibsiz", reply_markup=main_btn)
...
async def handle_rampage_time(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(259083453, f"Sizdan shu vaqt uchun - {message.text}\n"
                                  f"Bu foydalanuvchi ruxsat so'radingiz - {message.from_user.id}", reply_markup=admin_check_ruxsat)
